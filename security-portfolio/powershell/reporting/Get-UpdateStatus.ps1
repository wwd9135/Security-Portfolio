<#
.SYNOPSIS
    Read-only Windows Update compatibility and failure diagnostic.

.DESCRIPTION
    Collects 11 sections of Windows Update diagnostic data from the local machine
    and writes them to the console. Makes no changes; no administrator rights
    required for most sections (sections 3, 7, 8, 10, 11 may need elevation).

    Sections:
      0  Context (hostname, user, date)
      1  Build version (compare to latest CU)
      2  Uptime and pending-reboot flags (CBS / WU / FileRename)
      3  WU service status (wuauserv, bits, cryptsvc, UsoSvc, DoSvc)
      4  Policy / WSUS / deferral settings
      5  Last successful detect / download / install times
      6  Top 10 most recently installed hotfixes
      7  Last 15 WU history entries with HRESULT
      8  Current update backlog (not-installed applicable updates)
      9  C: drive free space
      10 WU client errors and warnings (last 15)
      11 Network connectivity to WU / WSUS endpoints

.NOTES
    Designed for pasting into a remote PowerShell session (RDP, PSRemoting)
    and collecting full output for triage. No parameters required.

.EXAMPLE
    .\Get-UpdateStatus.ps1
    Runs all sections and prints results to the console.
#>
...
function Invoke-Check ([string]$Label, [scriptblock]$Block) {
    try { & $Block }
    catch { Write-Output ("[ERROR] {0}: {1}" -f $Label, $_.Exception.Message) }
}
...
Write-Section '4. POLICY / WSUS / DEFERRALS'
Invoke-Check 'policy' {
    $p = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate'
    if (Test-Path $p) {
        Get-ItemProperty $p, "$p\AU" -ErrorAction SilentlyContinue |
            Select-Object WUServer, UseWUServer, TargetReleaseVersion, TargetReleaseVersionInfo,
                ProductVersion, DeferQualityUpdates, DeferQualityUpdatesPeriodInDays,
                PauseQualityUpdatesStartTime, BranchReadinessLevel, NoAutoUpdate, AUOptions |
...
Invoke-Check 'times' {
    foreach ($phase in 'Detect', 'Download', 'Install') {
        $v = (Get-ItemProperty "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\WindowsUpdate\Auto Update\Results\$phase" `
            -ErrorAction SilentlyContinue).LastSuccessTime
    ...
    if ($n -gt 0) {
        $searcher.QueryHistory(0, $n) | ForEach-Object {
            $result = switch ($_.ResultCode) {
                2 { 'OK' } 3 { 'OK-errs' } 4 { 'FAILED' } 5 { 'Abort' } default { $_.ResultCode }
            }
            [pscustomobject]@{
                Date    = $_.Date
                Result  = $result
...
Write-Section '8. BACKLOG (1-5 min; error here = cannot reach update source)'
...
Invoke-Check 'network' {
    ...
    if ($wu) {
        ...
        'No WSUS configured — device uses Windows Update online.'
    }
