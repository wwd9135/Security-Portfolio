# DCU Driver Auto-Update - Proactive Remediation REMEDIATION
# -------------------------------------------------------------------
# Deletes the heartbeat value so the Win32 app detection flips to "not installed"
# and Intune re-runs the PSADT app, which re-scans/applies driver updates and
...
try {
    if (Get-ItemProperty -Path $RegPath -Name $RegName -ErrorAction SilentlyContinue) {
        ...
        # Already absent = already in the desired post-remediation state.
